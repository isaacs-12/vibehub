...
**Desktop:** Open a folder that has a `.vibe/` directory (or run `vibe init` in a repo first), then use **Open Project** in the top bar so the app is “connected.” Chat and Vibe compile need a Gemini API key: put `GEMINI_API_KEY=your-key` in the repo `.env`; `make desktop` loads `.env` automatically.
